"""
运行 IB 下载的多周期 K 线，通过 chanlun_quant BacktestRunner 演示回测。
默认使用 StructureAnalyzer；如果你有自定义策略，只需替换 analyzer 或 trade_engine。
"""
...
def load_bars(symbol: str, freqs: List[str]) -> Dict[str, List[Bar]]:
    """从 SQLite 数据库读取指定标的的多周期 K 线。"""
    ...
    try:
        for freq in freqs:
            cursor = conn.execute(
                f"SELECT dt, o, h, l, c, v FROM {table} WHERE f=? AND code=? ORDER BY dt ASC",
                (freq, symbol),
            )
...
def main() -> None:
    symbol = "AAPL"
    freqs = ["1d", "30m", "5m"]
    bars_by_level = load_bars(symbol, freqs)
    ...
    if result.trades:
        ...
        print("最近一笔交易:", last_trade.signal, last_trade.order_result)
