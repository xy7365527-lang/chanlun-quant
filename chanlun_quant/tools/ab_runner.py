# file: chanlun_quant/tools/ab_runner.py
...
class ArrayFeed:
    ...
    def get_bars(self, symbol: str, level: str) -> Dict[str, List[float]]:
        ...
        raw = self.data[level]
        return {key: take(values) for key, values in raw.items()}
...
def run_grid(
    ...
    for values in itertools.product(*switches.values()):
        ...
        rows.append(
            {
                **params,
                "last_remaining_cost": float(ledger.remaining_cost),
                "realized_total": float(ledger.realized_total),
            }
        )
