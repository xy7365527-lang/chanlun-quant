...
@dataclass
class SelectorAdvCfg:
    density_low: float = 0.8
    density_high: float = 3.0
    zs_cover_min: float = 0.08
    natr_low: float = 0.004
    natr_high: float = 0.025
    nesting_min_success: float = 0.35


@dataclass
class MonitorCfg:
    max_structure_issues: int = 20
    max_envelope_jump: float = 0.15
    warn_on_ai_error: bool = True
...
@dataclass
class SynergyCfg:
    seg_div_sell_suppress_pen: bool = True
    prefer_segment_when_conflict: bool = True
    enforce_net_direction: bool = True
...
class Config:
    ...
    evidence_calib_path: str | None = None
    ...
    def from_env(cls, **overrides) -> "Config":
        ...
        calib_path = env("CLQ_EVIDENCE_CALIB_PATH")
        if calib_path:
            instance.evidence_calib_path = calib_path
