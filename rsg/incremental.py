...
from .build import _calc_segment_macd, _detect_zhongshu, _divergence_between, build_level_pens_segments
...
class RSGBuilder:
    ...
    _edges: Dict[Level, List[Dict[str, Any]]] = field(default_factory=dict)
    ...
    def rebuild_level(self, level: Level) -> None:
        ...
        highs = bars.get("high", [])
        lows = bars.get("low", [])
        macd_hist = bars.get("macd", [])
        zipped = [{"close": c, "high": h, "low": l} for c, h, l in zip(closes, highs, lows)]
        pens, segs, edges = build_level_pens_segments(zipped, level, macd_hist)
    ...
    def ingest(self, level_bars: Dict[Level, Dict[str, List[float]]]) -> RSG:
        ...
        for level in ordered_levels:
            ...
            rsg.edges.extend(self._edges.get(level, []))
